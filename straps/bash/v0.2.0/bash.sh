...
function strapped_bash() {
	...
	local __deps="echo mkdir touch exec ln "
	local __resp
...
"
	...
	# Declaring local variables for the 'ln' routine
	local symbolic
	local target
	local name
	...
	# performing functionality for routine 'ln'
	for ((i=0; i<$( ysh -T "${input}" -c ln ); i++)); do

		# Getting fields for routine 'ln'
		symbolic=$( ysh -T "${input}" -l ln -i ${i} -Q symbolic )
		target=$( ysh -T "${input}" -l ln -i ${i} -Q target )
		name=$( ysh -T "${input}" -l ln -i ${i} -Q name )

		# Writing message for routine 'ln'
		pretty_print ":info:" "🔗 linking ${target} to ${name}"

		# Executing the command(s) for routine 'ln'
		run_command "if [ ${symbolic} == 'true' ]; then ln -s ${target} ${name}; else ln ${target} ${name}; fi"
	done
